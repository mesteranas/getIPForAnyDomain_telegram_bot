...
async def start(update,contextt):
    ...
    await message.Sendmessage(chat_id=info.id,text="welcome " + str(info.first_name) + "to this bot. send a domain to get IP",reply_markup=keyboard)
async def handleMessages(update,context):
    info=update.effective_user
    id=await message.Sendmessage(info.id,"please wait")
    try:
        r=socket.gethostbyname(update.message.text)
    except:
        r="error"
    await message.Editmessage(info.id,r,id)
...
<a href="https://Github.com/mesteranas/getIPForAnyDomain_telegram_bot">visite project on Github</a>
"""
    info=update.effective_user
